...
signal on_websocket_string(packet: String)
signal on_websocket_data(packet: PackedByteArray)
		...
		if peer_state == WebSocketPeer.STATE_OPEN:
			while peer.get_available_packet_count():
				...
				if peer.was_string_packet():
					...
					on_websocket_string.emit(packet_text)
				else:
					print("< Got binary data from peer %d: %d" % [peer_id, packet.size()])
					# Echo the packet back.
					#peer.send(packet)
					on_websocket_data.emit(packet)
